...
"import qualified Control.Concurrent as AUX_NGX\n"
"import qualified Control.Concurrent.Async as AUX_NGX\n"
"import qualified Data.Bifunctor as AUX_NGX\n"
...
"newtype AUX_NGX_ServiceSomeInterrupt = AUX_NGX_ServiceSomeInterrupt String\n\n"

"instance AUX_NGX.Exception AUX_NGX_ServiceSomeInterrupt\n"
"instance Show AUX_NGX_ServiceSomeInterrupt where\n"
"    show (AUX_NGX_ServiceSomeInterrupt s) = s\n\n"
...
"aux_ngx_forceValue :: AUX_NGX.NFData a => IO a -> IO a\n"
"aux_ngx_forceValue = (>>= AUX_NGX.evaluate . AUX_NGX.force)\n"
"{-# SPECIALIZE INLINE aux_ngx_forceValue ::\n"
"    IO AUX_NGX_BSL.ByteString -> IO AUX_NGX_BSL.ByteString #-}\n"
"{-# SPECIALIZE INLINE aux_ngx_forceValue ::\n"
"    IO (AUX_NGX_BSL.ByteString, Bool) -> IO (AUX_NGX_BSL.ByteString, Bool) #-}"
"\n"
"{-# SPECIALIZE INLINE aux_ngx_forceValue ::\n"
...
"    let (s', n) = AUX_NGX_BSL.foldlChunks (flip (,) . succ . snd)\n"
"            (AUX_NGX_BS.empty, 0) s\n"
...
"                            _ -> case AUX_NGX.fromException e of\n"
"                                Just (AUX_NGX_ServiceSomeInterrupt _) -> 5\n"
"                                _ -> 1\n"
...
"aux_ngx_isEINTR = aux_ngx_isIOError AUX_NGX.eINTR\n"
"{-# INLINE aux_ngx_isEINTR #-}\n\n"

"aux_ngx_isEDEADLK :: AUX_NGX.IOError -> Bool\n"
"aux_ngx_isEDEADLK = aux_ngx_isIOError AUX_NGX.eDEADLK\n"
"{-# INLINE aux_ngx_isEDEADLK #-}\n\n"
...
"    AUX_NGX.mask_ $ AUX_NGX.async\n"
"    (do\n"
"        (s, (r, exiting)) <- aux_ngx_safeAsyncYYHandler $\n"
"            AUX_NGX.second (0, ) <$> aux_ngx_forceValue a\n"
"        aux_ngx_pokeLazyByteString s p pl spd\n"
"        AUX_NGX.poke pr r\n"
"        if exiting\n"
...
"                       -- BEWARE: using POSIX advisory file locks (F_SETLKW)\n"
"                       then acquireLock fdlk 7\n"
"                       else return False\n"
"        if exiting\n"
...
"                AUX_NGX.interruptible $ (, False) <$> f x' fstRun\n"
"    ) fd efd\n"
"    where acquireLock lk cmd = snd <$>\n"
"              AUX_NGX.iterateUntil fst\n"
"              (AUX_NGX.interruptible\n"
"                   (aux_ngx_safeWaitToSetLock lk cmd >> return (True, False))"
"\n"
"               `AUX_NGX.catches`\n"
"               [AUX_NGX.Handler $ \\e ->\n"
"                   if aux_ngx_isEINTR e\n"
...
"                           -- handlers\n"
"                           exiting <-\n"
"                               (AUX_NGX.threadDelay 500000 >> return False)\n"
"                               `AUX_NGX.catches`\n"
"                               [AUX_NGX.Handler $ return .\n"
"                                   (== AUX_NGX_WorkerProcessIsExiting)\n"
"                               ,AUX_NGX.Handler\n"
"                                   (const $ return False ::\n"
"                                       AUX_NGX.SomeException -> IO Bool\n"
"                                   )\n"
"                               ]\n"
"                           if exiting\n"
"                               then return (True, True)\n"
"                               else if aux_ngx_isEDEADLK e\n"
"                                        then return (False, False)\n"
"                                        else AUX_NGX.throwIO $\n"
"                                            AUX_NGX_ServiceSomeInterrupt $\n"
"                                                show e\n"
"               ,AUX_NGX.Handler $\n"
"                   return . (True, ) . (== AUX_NGX_WorkerProcessIsExiting)\n"
"               ,AUX_NGX.Handler\n"
"                   (AUX_NGX.throwIO . AUX_NGX_ServiceSomeInterrupt . show ::\n"
"                       AUX_NGX.SomeException -> IO (Bool, Bool)\n"
"                   )\n"
"               ]\n"
"              )\n\n"
...
"        x' <- AUX_NGX_BS.unsafePackCStringLen (x, n)\n"
"        AUX_NGX.interruptible $ (, False) <$> f b' x'\n"
"    ) fd efd\n\n"
...
"    (do\n"
"        x' <- AUX_NGX_BS.unsafePackCStringLen (x, n)\n"
"        (s, ct, fromIntegral -> st, rhs) <- AUX_NGX.interruptible $\n"
"            aux_ngx_forceValue $ f x'\n"
...
"        (s, ct, fromIntegral -> st, rhs) <- aux_ngx_forceValue $\n"
"            f <$> AUX_NGX_BS.unsafePackCStringLen (x, n)\n"
"        lct <- aux_ngx_pokeContentTypeAndStatus ct pct plct pst st\n"
...
"    aux_ngx_safeHandler pct pst $ do\n"
"        (s, ct, fromIntegral -> st) <- aux_ngx_forceValue $\n"
"            f <$> AUX_NGX_BS.unsafePackCStringLen (x, n)\n"
...
"-- FIXME: positions of fields in struct flock must depend on machine arch\n"
...
"aux_ngx_safeWaitToSetLock :: AUX_NGX.Fd -> AUX_NGX.CInt -> IO ()\n"
"aux_ngx_safeWaitToSetLock (AUX_NGX.Fd fd) cmd =\n"
"    aux_ngx_allocaLock (AUX_NGX.WriteLock, AUX_NGX.AbsoluteSeek, 0, 0) $\n"
"        \\p_flock -> AUX_NGX.throwErrnoIfMinus1_ \"safeWaitToSetLock\" $\n"
"            aux_ngx_safe_c_fcntl_lock fd cmd p_flock\n\n"
...
"ngxCachedPid :: IO AUX_NGX.CPid\n"
"ngxCachedPid = AUX_NGX.readIORef aux_ngx_ngxCachedPidStore\n\n"
...
"foreign export ccall ngxExportSetCachedPid ::\n"
"    AUX_NGX.CPid -> IO ()\n"
"ngxExportSetCachedPid :: AUX_NGX.CPid -> IO ()\n"
"ngxExportSetCachedPid = AUX_NGX.writeIORef aux_ngx_ngxCachedPidStore\n\n"
...
"aux_ngx_ngxCachedPidStore :: AUX_NGX.IORef AUX_NGX.CPid\n"
"aux_ngx_ngxCachedPidStore =\n"
"    AUX_NGX.unsafePerformIO $ AUX_NGX.newIORef (-1)\n"
"{-# NOINLINE aux_ngx_ngxCachedPidStore #-}\n\n"
);
