...
The initialization hook can be used in the *mread* trick for passing sensitive
data from the master process to worker processes. When directive *haskell
program_options* contains arguments with prefix *--mread:* followed by an
argument with a path (e.g. *--mread:ca /path/to/ca.crt*) then the master process
will read the file found by the path and substitute its content in the second
argument instead of the path. The data keeps alive during the call to the
