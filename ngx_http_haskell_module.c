...
"    `AUX_NGX.finally`\n"
"    ((if efd\n"
"          then AUX_NGX.void $ AUX_NGX_BS.unsafeUseAsCString\n"
"                  aux_ngx_asyncIOFlag8b\n"
"                      (\\(AUX_NGX.castPtr -> s) -> AUX_NGX.fdWriteBuf fd s 8)"
"\n"
"          else AUX_NGX_BS.unsafeUseAsCString\n"
"                  aux_ngx_asyncIOFlag1b\n"
"                      (\\(AUX_NGX.castPtr -> s) -> AUX_NGX.fdWriteBuf fd s 1) "
">>\n"
"                          AUX_NGX.closeFd fd\n"
                                     ...
                                     uintptr_t  data)
{
    ...
    if (found_idx != NGX_ERROR) {
        ...
        if (shm_vars[found_idx].data.result.len == 0) {
            ngx_shmtx_unlock(&shpool->mutex);
            goto update_var;
        }
...
ngx_http_haskell_service_async_event(ngx_event_t *ev)
{
    ...
    if (service_code_var->async_data.result.len == 0) {
        if (var->data != NULL) {
            ngx_slab_free_locked(shpool, var->data);
        }
        ngx_str_null(var);
